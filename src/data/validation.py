...
raw_data_schema = pa.DataFrameSchema(
    {
        "Unnamed: 0": Column(int, nullable=True, required=False),
        "SeriousDlqin2yrs": Column(int, Check.isin([0, 1]), nullable=False),
        "RevolvingUtilizationOfUnsecuredLines": Column(float, Check.ge(0.0), nullable=False),
        ...
        "DebtRatio": Column(float, Check.ge(0.0), nullable=False),
        "MonthlyIncome": Column(float, Check.ge(0.0), nullable=True),
        ...
        "TimeCreditLine": Column(int, Check.ge(0), nullable=True, required=False), # Handling variations if present
        "NumberDependingPersons": Column(float, Check.ge(0.0), nullable=True),
    },
